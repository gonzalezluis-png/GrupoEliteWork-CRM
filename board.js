...
function switchSettingsTab(tab) {
  ...
  if (tab === 'marketing-stats') { loadMarketingStats(); }
}

// ── Marketing Stats ──────────────────────────────────────────────────────────
let _mstatsData = null;
let _mstatsSortField = 'total';
let _mstatsSortDir   = -1;
let _mstatsCharts    = {};

function _mDestroyChart(id) {
  if (_mstatsCharts[id]) { _mstatsCharts[id].destroy(); delete _mstatsCharts[id]; }
}

async function loadMarketingStats() {
  const loading = document.getElementById('mstats-loading');
  const content = document.getElementById('mstats-content');
  if (!loading || !content) return;
  loading.style.display = 'block';
  content.style.display = 'none';
  try {
    const r = await fetch('https://elite-reclutamiento-production.up.railway.app/api/stats?token=gew_stats_2026');
    if (!r.ok) throw new Error('HTTP ' + r.status);
    _mstatsData = await r.json();
    _renderMarketingStats(_mstatsData);
    loading.style.display = 'none';
    content.style.display = 'block';
  } catch(e) {
    loading.innerHTML = `<span style="color:var(--red)">Error cargando stats: ${e.message}</span>`;
  }
}

function _mPct(a, b) { return b > 0 ? Math.round(a / b * 100) : 0; }
function _mPill(p) {
  if (p >= 50) return `<span style="background:rgba(34,197,94,.15);color:#22c55e;padding:2px 7px;border-radius:99px;font-size:11px;font-weight:600;">${p}%</span>`;
  if (p >= 20) return `<span style="background:rgba(245,158,11,.15);color:#f59e0b;padding:2px 7px;border-radius:99px;font-size:11px;font-weight:600;">${p}%</span>`;
  return `<span style="background:rgba(239,68,68,.15);color:#ef4444;padding:2px 7px;border-radius:99px;font-size:11px;font-weight:600;">${p}%</span>`;
}

function _renderMarketingStats(s) {
  // KPIs
  const kpiW = _mPct(s.totals.webinar, s.totals.leads);
  const kpiE = _mPct(s.totals.entrevista, s.totals.leads);
  document.getElementById('mstats-kpis').innerHTML = [
    { val: s.totals.leads,    lbl: 'Total Leads',       color: '#4d7cff' },
    { val: s.totals.webinar,  lbl: `Webinar (${kpiW}%)`, color: '#f59e0b' },
    { val: s.totals.entrevista, lbl: `Entrevista (${kpiE}%)`, color: '#22c55e' },
    { val: Object.keys(s.campaigns).length, lbl: 'Campañas activas', color: '#a855f7' },
  ].map(k => `
    <div style="background:var(--bg2,var(--bg));border:1px solid var(--border);border-radius:10px;padding:14px 16px;">
      <div style="font-size:28px;font-weight:700;color:${k.color};">${k.val}</div>
      <div style="font-size:12px;color:var(--text2);margin-top:4px;">${k.lbl}</div>
    </div>`).join('');

  // Funnel
  const funnelStages = [
    { id: 'postulados-meta',          label: '1. Nuevo lead (Form)',    color: '#4d7cff' },
    { id: 'postulados-whatsapp-meta', label: '2. Nuevo lead (WA)',      color: '#4d7cff' },
    { id: 'en-webinar',               label: '3. En Webinar',           color: '#f59e0b' },
    { id: 'entrevistas-generales',    label: '4. En Entrevista',        color: '#22c55e' },
    { id: 'vendidos',                 label: '5. Contratados',          color: '#a855f7' },
    { id: 'no-interesados-no-califica', label: '✗ No califica',         color: '#ef4444' },
  ];
  document.getElementById('mstats-funnel').innerHTML = funnelStages.map(st => {
    const cnt = s.pipeline[st.id] || 0;
    const p   = _mPct(cnt, s.totals.leads);
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);">
      <div style="font-size:12px;">${st.label}</div>
      <div style="text-align:right;">
        <span style="font-size:18px;font-weight:700;color:${st.color};">${cnt}</span>
        <span style="font-size:11px;color:var(--text2);margin-left:4px;">(${p}%)</span>
      </div>
    </div>`;
  }).join('');

  // Daily chart
  _mDestroyChart('daily');
  if (typeof Chart !== 'undefined') {
    const last30 = s.daily.slice(-30);
    _mstatsCharts['daily'] = new Chart(document.getElementById('mstats-chart-daily'), {
      type: 'bar',
      data: {
        labels: last30.map(d => d.date.slice(5)),
        datasets: [{ data: last30.map(d => d.count), backgroundColor: 'rgba(77,124,255,.7)', borderRadius: 3 }]
      },
      options: { plugins: { legend: { display: false } }, scales: {
        x: { ticks: { color: '#5a6076', font: { size: 9 } }, grid: { color: 'rgba(255,255,255,.04)' } },
        y: { ticks: { color: '#5a6076', font: { size: 9 } }, grid: { color: 'rgba(255,255,255,.04)' } }
      }}
    });

    // Campaigns bar chart
    const topCamp = Object.entries(s.campaigns).sort((a,b) => b[1].total - a[1].total).slice(0,10);
    _mDestroyChart('campaigns');
    _mstatsCharts['campaigns'] = new Chart(document.getElementById('mstats-chart-campaigns'), {
      type: 'bar',
      data: {
        labels: topCamp.map(([k]) => k.length > 35 ? k.slice(0,33)+'…' : k),
        datasets: [
          { label: 'Total',      data: topCamp.map(([,v]) => v.total),      backgroundColor: 'rgba(77,124,255,.7)',  borderRadius: 3 },
          { label: 'Webinar',    data: topCamp.map(([,v]) => v.webinar),    backgroundColor: 'rgba(245,158,11,.7)',  borderRadius: 3 },
          { label: 'Entrevista', data: topCamp.map(([,v]) => v.entrevista), backgroundColor: 'rgba(34,197,94,.7)',   borderRadius: 3 },
        ]
      },
      options: { indexAxis: 'y',
        plugins: { legend: { labels: { color: '#9aa0b3', font: { size: 11 } } } },
        scales: {
          x: { ticks: { color: '#5a6076', font: { size: 9 } }, grid: { color: 'rgba(255,255,255,.04)' } },
          y: { ticks: { color: '#9aa0b3', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' } }
        }
      }
    });
  }
  ...
  // Campaigns table
  mstatsRenderCampaignsTable();

  // Ads
  const topAds = Object.entries(s.ads).sort((a,b) => b[1].total - a[1].total).slice(0,15);
  document.getElementById('mstats-body-ads').innerHTML = topAds.map(([name, v]) => {
    const p = _mPct(v.webinar, v.total);
    return `<tr style="border-bottom:1px solid var(--border);">
      <td style="padding:6px 8px;font-size:11px;max-width:160px;word-break:break-word;">${name}</td>
      <td style="padding:6px 8px;font-size:12px;font-weight:600;">${v.total}</td>
      <td style="padding:6px 8px;font-size:12px;">${v.webinar}</td>
      <td style="padding:6px 8px;">${_mPill(p)}</td>
    </tr>`;
  }).join('') || '<tr><td colspan="4" style="padding:12px;color:var(--text2);font-size:12px;">Sin datos</td></tr>';

  // Forms
  const topForms = Object.entries(s.forms).sort((a,b) => b[1].total - a[1].total);
  document.getElementById('mstats-body-forms').innerHTML = topForms.map(([name, v]) => {
    const p = _mPct(v.webinar, v.total);
    return `<tr style="border-bottom:1px solid var(--border);">
      <td style="padding:6px 8px;font-size:11px;max-width:160px;word-break:break-word;">${name}</td>
      <td style="padding:6px 8px;font-size:12px;font-weight:600;">${v.total}</td>
  ...
  const ga = document.getElementById('mstats-generated-at');
  if (ga) ga.textContent = 'Actualizado: ' + new Date(s.generatedAt).toLocaleString('es-MX');
}

function mstatsRenderCampaignsTable() {
  if (!_mstatsData) return;
  const entries = Object.entries(_mstatsData.campaigns)
    .sort((a,b) => _mstatsSortDir * (b[1][_mstatsSortField] - a[1][_mstatsSortField]));
  document.getElementById('mstats-body-campaigns').innerHTML = entries.map(([name, v]) => {
    const pw = _mPct(v.webinar, v.total);
    const pe = _mPct(v.entrevista, v.total);
    return `<tr style="border-bottom:1px solid var(--border);">
      <td style="padding:6px 10px;font-size:11px;max-width:260px;word-break:break-word;">${name}</td>
      <td style="padding:6px 10px;font-size:13px;font-weight:600;">${v.total}</td>
      <td style="padding:6px 10px;font-size:13px;">${v.webinar}</td>
      <td style="padding:6px 10px;">${_mPill(pw)}</td>
      <td style="padding:6px 10px;font-size:13px;">${v.entrevista}</td>
      <td style="padding:6px 10px;">${_mPill(pe)}</td>
    </tr>`;
  }).join('') || '<tr><td colspan="6" style="padding:12px;color:var(--text2);">Sin datos de campaña</td></tr>';
}

function mstatsSortCampaigns(field) {
  if (_mstatsSortField === field) _mstatsSortDir *= -1;
  else { _mstatsSortField = field; _mstatsSortDir = -1; }
  mstatsRenderCampaignsTable();
}
